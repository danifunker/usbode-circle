...
SSD1306USBConfigPage::SSD1306USBConfigPage(CSSD1306GfxDisplay* display, C2DGraphics* graphics)
    : m_Display(display),
      m_Graphics(graphics) {
    config = static_cast<ConfigService*>(CScheduler::Get()->GetTask("configservice"));
}

SSD1306USBConfigPage::~SSD1306USBConfigPage() {
    LOGNOTE("USBConfigPage starting");
}

void SSD1306USBConfigPage::OnEnter() {
...
void SSD1306USBConfigPage::OnExit() {
    m_ShouldChangePage = false;
}

bool SSD1306USBConfigPage::shouldChangePage() {
    return m_ShouldChangePage;
}

const char* SSD1306USBConfigPage::nextPageName() {
    return "configpage";
}

void SSD1306USBConfigPage::OnButtonPress(Button button) {
...
void SSD1306USBConfigPage::MoveSelection(int delta) {
...
void SSD1306USBConfigPage::Refresh() {
    // TODO We shouldn't redraw everything!
    // Draw();
}

void SSD1306USBConfigPage::SaveAndReboot() {
