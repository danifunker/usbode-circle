...
SSD1306SetupPage::SSD1306SetupPage(CSSD1306GfxDisplay* display, C2DGraphics* graphics)
    : m_Display(display), m_Graphics(graphics) {
    m_statusText[0] = '\0';
}

SSD1306SetupPage::~SSD1306SetupPage() {}

void SSD1306SetupPage::OnEnter() {
...
void SSD1306SetupPage::OnExit() {
    m_ShouldChangePage = false;
}

bool SSD1306SetupPage::shouldChangePage() {
    return m_ShouldChangePage;
}

const char* SSD1306SetupPage::nextPageName() {
    return "homepage";
}

void SSD1306SetupPage::OnButtonPress(Button buttonId) {
    // Ignore buttons during setup
}

void SSD1306SetupPage::Refresh() {
    m_refreshCounter++;
