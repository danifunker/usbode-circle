...
SSD1306ImagesPage::SSD1306ImagesPage(CSSD1306GfxDisplay* display, C2DGraphics* graphics)
    : m_Display(display),
      m_Graphics(graphics) {
...
SSD1306ImagesPage::~SSD1306ImagesPage() {
}

void SSD1306ImagesPage::OnEnter() {
...
void SSD1306ImagesPage::OnExit() {
    m_ShouldChangePage = false;
}

bool SSD1306ImagesPage::shouldChangePage() {
    return m_ShouldChangePage;
}

const char* SSD1306ImagesPage::nextPageName() {
    return m_NextPageName;
}

void SSD1306ImagesPage::OnButtonPress(Button button) {
...
void SSD1306ImagesPage::MoveSelection(int delta) {
...
void SSD1306ImagesPage::NavigateToFolder(const char* path) {
    if (!path) return;
...
void SSD1306ImagesPage::NavigateUp() {
...
size_t SSD1306ImagesPage::GetVisibleCount() {
    ConfigService* config = ConfigService::Get();
    bool flatFileList = config ? config->GetFlatFileList() : false;
...
bool SSD1306ImagesPage::IsParentDirEntry(size_t visibleIndex) {
    ConfigService* config = ConfigService::Get();
    bool flatFileList = config ? config->GetFlatFileList() : false;
    bool isRoot = (m_CurrentPath[0] == '\0');
...
size_t SSD1306ImagesPage::GetCacheIndex(size_t visibleIndex) {
    ConfigService* config = ConfigService::Get();
    bool flatFileList = config ? config->GetFlatFileList() : false;
    bool isRoot = (m_CurrentPath[0] == '\0');
    size_t pathLen = strlen(m_CurrentPath);
...
const char* SSD1306ImagesPage::GetDisplayName(size_t visibleIndex) {
...
void SSD1306ImagesPage::DrawText(unsigned nX, unsigned nY, T2DColor Color, const char* pText,
...
void SSD1306ImagesPage::DrawTextScrolled(unsigned nX, unsigned nY, T2DColor Color, const char* pText,
...
void SSD1306ImagesPage::RefreshScroll() {
    size_t visibleCount = GetVisibleCount();
