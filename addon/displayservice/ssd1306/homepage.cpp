...
SSD1306HomePage::SSD1306HomePage(CSSD1306GfxDisplay* display, C2DGraphics* graphics)
    : m_Display(display),
      m_Graphics(graphics) {
...
SSD1306HomePage::~SSD1306HomePage() {
}

void SSD1306HomePage::OnEnter() {
...
void SSD1306HomePage::OnExit() {
    m_ShouldChangePage = false;
}

bool SSD1306HomePage::shouldChangePage() {
    return m_ShouldChangePage;
}

const char* SSD1306HomePage::nextPageName() {
    return m_NextPageName;
}

void SSD1306HomePage::OnButtonPress(Button button) {
...
const char* SSD1306HomePage::GetVersionString() {
    return CGitInfo::Get()->GetShortVersionString();
}

const char* SSD1306HomePage::GetCurrentImagePath() {
...
const char* SSD1306HomePage::GetUSBSpeed() {
...
void SSD1306HomePage::Draw() {
	...
	for (int x = -4; x <= 4; x++) {
		...
		if (px < CSSD1306GfxDisplay::OLED_WIDTH && py < CSSD1306GfxDisplay::OLED_HEIGHT) {
...
void SSD1306HomePage::DrawTextScrolled(unsigned nX, unsigned nY, T2DColor Color, const char* pText,
...
void SSD1306HomePage::RefreshISOScroll() {
    size_t pathLen = strlen(pISOPath);
    int fullTextPx = (int)pathLen * m_ISOCharWidth;
