...
SSD1306SoundConfigPage::SSD1306SoundConfigPage(CSSD1306GfxDisplay* display, C2DGraphics* graphics)
    : m_Display(display),
      m_Graphics(graphics) {
    config = static_cast<ConfigService*>(CScheduler::Get()->GetTask("configservice"));
}

SSD1306SoundConfigPage::~SSD1306SoundConfigPage() {
    LOGNOTE("SoundConfigPage starting");
}

void SSD1306SoundConfigPage::OnEnter() {
...
void SSD1306SoundConfigPage::OnExit() {
    m_ShouldChangePage = false;
}

bool SSD1306SoundConfigPage::shouldChangePage() {
    return m_ShouldChangePage;
}

const char* SSD1306SoundConfigPage::nextPageName() {
    return "configpage";
}

void SSD1306SoundConfigPage::OnButtonPress(Button button) {
...
void SSD1306SoundConfigPage::MoveSelection(int delta) {
...
void SSD1306SoundConfigPage::Refresh() {
    // TODO We shouldn't redraw everything!
    // Draw();
}

void SSD1306SoundConfigPage::SaveAndReboot() {
...
void SSD1306SoundConfigPage::DrawConfirmation(const char* message) {
...
void SSD1306SoundConfigPage::Draw() {
    size_t fileCount = sizeof(options) / sizeof(options[0]);
    if (fileCount == 0) return;
