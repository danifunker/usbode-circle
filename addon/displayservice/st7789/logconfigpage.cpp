...
ST7789LogConfigPage::ST7789LogConfigPage(CST7789Display* display, C2DGraphics* graphics)
    : m_Display(display),
      m_Graphics(graphics) {
    config = static_cast<ConfigService*>(CScheduler::Get()->GetTask("configservice"));
}

ST7789LogConfigPage::~ST7789LogConfigPage() {
    LOGNOTE("LogConfigPage starting");
}

void ST7789LogConfigPage::OnEnter() {
    LOGNOTE("Drawing LogConfigPage");
    m_SelectedIndex = config->GetLogLevel();
    Draw();
}

void ST7789LogConfigPage::OnExit() {
    m_ShouldChangePage = false;
}

bool ST7789LogConfigPage::shouldChangePage() {
    return m_ShouldChangePage;
}

const char* ST7789LogConfigPage::nextPageName() {
    return "configpage";
}

void ST7789LogConfigPage::OnButtonPress(Button button) {
...
void ST7789LogConfigPage::MoveSelection(int delta) {
...
void ST7789LogConfigPage::Refresh() {
    // TODO We shouldn't redraw everything!
    // Draw();
}

void ST7789LogConfigPage::SaveAndReboot() {
...
void ST7789LogConfigPage::DrawConfirmation(const char* message) {
    m_Graphics->ClearScreen(COLOR2D(255, 255, 255));

    // Draw header bar with blue background
    const char* pTitle = "Logging";
...
void ST7789LogConfigPage::Draw() {
    ...
    // Draw header bar with blue background
    const char* pTitle = "Logging Config";
    m_Graphics->DrawRect(0, 0, m_Display->GetWidth(), 30, COLOR2D(58, 124, 165));
    m_Graphics->DrawText(10, 8, COLOR2D(255, 255, 255), pTitle, C2DGraphics::AlignLeft);
...
void ST7789LogConfigPage::DrawNavigationBar(const char* screenType) {
