...
SH1106ClassicMacModePage::SH1106ClassicMacModePage(CSH1106Display* display, C2DGraphics* graphics)
    : m_Display(display),
      m_Graphics(graphics) {
    config = static_cast<ConfigService*>(CScheduler::Get()->GetTask("configservice"));
}

SH1106ClassicMacModePage::~SH1106ClassicMacModePage() {
...
void SH1106ClassicMacModePage::OnExit() {
    m_ShouldChangePage = false;
}

bool SH1106ClassicMacModePage::shouldChangePage() {
    return m_ShouldChangePage;
}

const char* SH1106ClassicMacModePage::nextPageName() {
    return "configpage";
}

void SH1106ClassicMacModePage::OnButtonPress(Button button) {
...
void SH1106ClassicMacModePage::MoveSelection(int delta) {
...
void SH1106ClassicMacModePage::Refresh() {
    // TODO We shouldn't redraw everything!
    // Draw();
}

void SH1106ClassicMacModePage::SaveAndReboot() {
...
void SH1106ClassicMacModePage::DrawConfirmation(const char* message) {
...
void SH1106ClassicMacModePage::Draw() {
    size_t fileCount = sizeof(options) / sizeof(options[0]);
    if (fileCount == 0) return;
