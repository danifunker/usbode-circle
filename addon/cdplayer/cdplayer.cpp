    ...
    while (true) {
        ...
        if (state == PLAYING) {
            ...
            while (m_ReadBytePos < max_read_limit) {
                ...
                u64 seekResult = m_pBinFileDevice->Seek(m_ReadBytePos);
                if (seekResult == (u64)-1) {
                     LOGERR("Seek failed at %lu", (unsigned long)m_ReadBytePos);
                     // Yield and try again next loop - hope transient error or busy
                     break;
                }
