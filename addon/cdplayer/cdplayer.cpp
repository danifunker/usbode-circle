...
void CCDPlayer::Run(void) {
    ...
    while (true) {
        ...
        if (state == PLAYING) {
            ...
            u32 bytes_read_this_pass = 0;
            const u32 MAX_READ_BYTES_PER_PASS = 16 * SECTOR_SIZE; // Limit read to 16 sectors per Run() to avoid blocking
            ...
            while (m_ReadBytePos < max_read_limit) {
                ...
                // Optimization: Don't read too much in one go to keep system responsive
                // and minimize start latency.
                if (bytes_read_this_pass >= MAX_READ_BYTES_PER_PASS) break;
                ...
                // Further limit single read operation to our pass limit if needed
                if (to_read > MAX_READ_BYTES_PER_PASS - bytes_read_this_pass) {
                    to_read = MAX_READ_BYTES_PER_PASS - bytes_read_this_pass;
                    // Re-align after capping
                    if (to_read >= SECTOR_SIZE) {
                        to_read = (to_read / SECTOR_SIZE) * SECTOR_SIZE;
                    }
                }
                ...
                bytes_read_this_pass += readCount;
