...
// Helper function to consolidate PLAY AUDIO logic
void CUSBCDGadget::HandlePlayAudio(u32 startLBA, u32 numBlocks)
{
    if (numBlocks == 0)
    {
        sendGoodStatus();
    ...
    if (startLBA >= leadout || (startLBA + numBlocks) > leadout)
    {
        MLOGERR("HandlePlayAudio", "LBA out of range (start=%u, count=%u, leadout=%u)",
                startLBA, numBlocks, leadout);
        setSenseData(0x05, 0x21, 0x00); // LOGICAL BLOCK ADDRESS OUT OF RANGE
        sendCheckCondition();
        return;
    }

    CUETrackInfo trackInfo = GetTrackInfoForLBA(startLBA);
    if (trackInfo.track_number == -1 || trackInfo.track_mode != CUETrack_AUDIO)
    {
        MLOGERR("HandlePlayAudio", "Not an audio track (track=%d, mode=%d)",
    ...
    if (!cdplayer)
    {
        MLOGERR("HandlePlayAudio", "CD Player service not available");
    ...
    if (startLBA == 0xffffffff)
    {
        cdplayer->Resume();
    }
    else
    {
        cdplayer->Play(startLBA, numBlocks);
    }
...
void CUSBCDGadget::HandleSCSICommand()
{
    ...
    case 0xBE: // READ CD -- bluescsi inspired
    {
        ...
        default:
            if (trackInfo.track_mode == CUETrack_AUDIO)
            {
                // **FIX: Handle audio with MCS properly**
                block_size = 2352;
                if (mcs == 0)
                {
                    // MCS=0 means all fields - full 2352 bytes
                    transfer_block_size = 2352;
                    skip_bytes = 0;
                }
                else
                {
                    ...
                    // **FIX: Fallback if MCS parsing returns 0**
            ...
            {
                // Data tracks - existing code is fine
                block_size = GetBlocksizeForTrack(trackInfo);
                transfer_block_size = GetSectorLengthFromMCS(mcs);
                skip_bytes = GetSkipBytesFromMCS(mcs);
    ...
    {
        bool is12byte = (m_CBW.CBWCB[0] == 0xA5);

        // Parse LBA start address (same for both)
        m_nblock_address = (u32)(m_CBW.CBWCB[2] << 24) | (u32)(m_CBW.CBWCB[3] << 16) |
                           (u32)(m_CBW.CBWCB[4] << 8) | m_CBW.CBWCB[5];

        // Parse block count (different byte positions)
        if (is12byte)
        {
            m_nnumber_blocks = (u32)(m_CBW.CBWCB[6] << 24) | (u32)(m_CBW.CBWCB[7] << 16) |
                               (u32)(m_CBW.CBWCB[8] << 8) | m_CBW.CBWCB[9];
        ...
        MLOGNOTE("CUSBCDGadget::HandleSCSICommand",
                 "PLAY AUDIO (%d) from LBA %lu for %lu blocks",
                 is12byte ? 12 : 10, m_nblock_address, m_nnumber_blocks);

        HandlePlayAudio(m_nblock_address, m_nnumber_blocks);
        break;
    }
