...
int CUSBCDGadget::GetMediumType()
{
    ...
    if (hasData && hasAudio)
    {
        CDROM_DEBUG_LOG("CUSBCDGadget::GetMediumType", "Medium type: Mixed");
        return 0x03; // Mixed
    }
    else if (hasData && !hasAudio)
    {
        CDROM_DEBUG_LOG("CUSBCDGadget::GetMediumType", "Medium type: Data");
        return 0x02; // Data
    }
    else if (hasAudio && !hasData)
    {
        CDROM_DEBUG_LOG("CUSBCDGadget::GetMediumType", "Medium type: Audio");
        return 0x01; // Audio
    }
    else
    {
        CDROM_DEBUG_LOG("CUSBCDGadget::GetMediumType", "Medium type: Unknown");
        return 0x00; // Unknown/none
    }
}
...
{
    // BUG FIX: Control/ADR byte must match track type
    ...
    {
        dest[4] = 0;
        LBA2MSF(track->data_start, &dest[5], false); // BUG: Was using track_start, should be data_start
    }
    else
    {
        // BUG FIX: Use data_start for TOC entries (not track_start)
...
{
    // BUG FIX: Control/ADR must distinguish audio vs data
    uint8_t control_adr = (track->track_mode == CUETrack_AUDIO) ? 0x10 : 0x14;
    ...
    dest[4] = 0x00;                // Min (unused)
    dest[5] = 0x00;                // Sec (unused)
    dest[6] = 0x00;                // Frame (unused)
    dest[7] = 0x00;                // HOUR (always 0)

    if (useBCD)
    {
        LBA2MSFBCD(track->data_start, &dest[8], false); // BUG: Was using track_start
    }
    else
    {
        LBA2MSF(track->data_start, &dest[8], false); // BUG: Was using track_start
    }
}
    ...
    while ((trackinfo = cueParser.next_track()) != nullptr)
    {
        ...
        if (startingTrack == 0 || trackinfo->track_number >= startingTrack)
        {
            ...
            CDROM_DEBUG_LOG("CUSBCDGadget::DoReadTOC",
                            ...
                            (trackinfo->track_mode == CUETrack_AUDIO) ? 0x10 : 0x14,
                            trackinfo->data_start);
            trackcount++;
        }
    }
...
void CUSBCDGadget::HandleSCSICommand()
{
    ...
    case 0xa8: // Read (12) - similar to READ(10) but with 32-bit block count
    {
        if (m_CDReady)
        {
            ...
            CDROM_DEBUG_LOG("CUSBCDGadget::HandleSCSICommand",
                            "READ(12): LBA=%u, blocks=%u, track=%d, block_size=%d, skip=%d",
                            m_nblock_address, m_nnumber_blocks,
                            trackInfo.track_number, block_size, skip_bytes);
    ...
    case 0x28: // Read (10)
    {
        if (m_CDReady)
        {
            ...
            // Transfer Block Size is the size of data to return to host
            // For READ(10), this is always 2048 bytes
            transfer_block_size = 2048;
            ...
            CDROM_DEBUG_LOG("CUSBCDGadget::HandleSCSICommand",
                            "READ(10): LBA=%u, blocks=%u, track=%d, block_size=%d, skip=%d",
                            m_nblock_address, m_nnumber_blocks,
                            trackInfo.track_number, block_size, skip_bytes);
