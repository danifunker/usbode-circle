...
// Mode Page 0x08 (Caching Mode Page)
struct ModePage0x08Data {
    u8 pageCodeAndPS;       // Bits 5-0: Page Code (0x08), Bit 7 PS
    u8 pageLength;          // 0x0A (length of fields after this one for 12 byte page)
    u8 WCE : 1;             // Write Cache Enable
    u8 MF : 1;              // Multiplication Factor
    u8 RCD : 1;             // Read Cache Disable
    u8 reserved_byte2_bits3_7 : 5;
    u8 prefetchControl;     // Was historically byte 3, various prefetch controls, often 0
    u16 disablePrefetchTransferLength; // Big Endian
    u16 minimumPrefetch;               // Big Endian
    u16 maximumPrefetch;               // Big Endian
    u16 maximumPrefetchCeiling;        // Big Endian
} PACKED;
#define SIZE_MODE_PAGE_0X08 (sizeof(ModePage0x08Data))
...
class CUSBCDGadget : public CDWUSBGadget  /// USB mass storage device gadget
{
   ...
   private:
    ...
    TUSBCDRequestSenseReply m_ReqSenseReply = {
        0x70, 0x00, 0x00, {0x0, 0x0, 0x0, 0x0}, 0x0A, {0x0, 0x0, 0x0, 0x0}, // bAddlSenseLen changed to 0x0A (10)
