...
Ever wanted a GoTek for CDs? The USBODE is an optical drive emulator that uses a Raspberry Pi Zero W to read image files from a Micro SD card, and acts as an optical drive through a USB cable. By default, it can load .ISO and .BIN/CUE images, and can be controlled over a web interface. It can read CD images, and some DVD images. Anything smaller than 4 GB should read, but has not been thoroughly tested.

## What can it do?
USBODE emulates an Optical Disc Drive over USB. If your retro computer supports USB, this project can act like a CD drive for it, and load and swap images on the fly. By emulating an optical drive with USBODE, you can:
- Store a collection of image files on the SD card and quickly switch between them.
  - USBODE supports ISO images, as well as BIN/CUE (with some limitations; see [the note below](#bincue-limitations)).
- Install and run CD-based games without the need for physical media. This includes multi-disc titles.
- Additionally, the USBODE can employ the [Waveshare OLED HAT](https://www.waveshare.com/wiki/1.3inch_OLED_HAT), which provides a way to directly control the device without the need for a web interface.
- It can also employ the [Pirate Audio Line Out/LCD HAT](https://shop.pimoroni.com/products/pirate-audio-line-out), which has all of the advantages of the Waveshare hat _and_ has a Line Out for CD Audio playback.

Note: Some forms of CD-ROM copy protection won’t work with USBODE.

## Requirements:
1. A Raspberry Pi Zero W or Zero 2 W (USBODE is optimized for the 2, but currently works on the 1 as well).
2. A MicroSD card. Cards up to 128 GB have been tested and work, larger cards might work as well. The system files are only a few MB, but images can be hundreds of MB each. Consider the size of your library when choosing a card. We suggest using a card marked A1 or A2 for best performance.
3. A setup computer with the ability to mount and format the MicroSD card. No special software is required, and any OS (Windows, MacOS, or Linux) will work.
  - Please note: in Windows 10 and below, the UI for the formatting utility has an arbitrary 32 GB limit for FAT32 partitions. For larger cards, we recommend using the command line DISKPART utility, the [Raspberry Pi Imager](https://www.raspberrypi.com/software/), or [FAT32FormatterGUI](https://www.softpedia.com/get/System/Hard-Disk-Utils/FAT32format-GUI.shtml).
...
# Recommendations
USBODE Version 2.0 and above support CD Audio. To take advantage of this, you’ll need a DAC. We suggest using the [Pirate Audio Line Out/LCD HAT](https://shop.pimoroni.com/products/pirate-audio-line-out), because it provides both a DAC and an LCD, making it easy to switch between different images.
...
3. If the “wpa_supplicant.conf” file is configured correctly, it should connect to your wifi network automatically, within 10 to 15 seconds of startup.
...
4. For the Pirate Audio HAT: The display will turn on a second or two after plugging in the Pi. The display will say “Not connected yet” until it connects to your wifi network, then that line will display the device’s IP address. The bottom of the display denotes what the buttons do.
5. For the Waveshare HAT: The display will turn on a second or two after plugging in the Pi. The display will say “Not connected" until it connects to your wifi network, then that line will display the device’s IP address.
6. Buttons: Key 1 enters a browsing mode, where you can see what images are on your SD card. Here, the directional stick on the left side can be moved up and down to go up or down the list of images, one at a time. Moving the stick left or right skips 5 images, making it easier to get to the bottom of a long list. Pressing the directional stick or Key 1 here will select that image and load it. Key 2 will back out of that screen without changing the image, and Key 3 does nothing currently.
...
### Using FTP: 
1. Select FTP as the protocol, not SFTP.
2. Enter the USBODE's IP address into the Host Name field.
3. Ensure the Port number is 21.
4. If available, check the "Anonymous login" box. Otherwise, enter "anonymous" as the username.
5. Navigate to the Images folder, and drop image files into it.
