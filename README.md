...
1.  One of the following Raspberry Pi SBCs:
    ...
    - 4B+ (support is not yet complete; requires a USB-C to USB-A cable)
...
On the USBODE homepage, click *Shutdown USBODE*. The LED indicator on the Pi will flash for several seconds and turn off.
...
- The image "usb-audio-sampler" can be used to test your Line-In settings and USBODE's audio settings. It can be safely deleted if space is at a premium.
